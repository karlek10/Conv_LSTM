...
class ConvLSTMCell(nn.Module):
    def __init__(self, input_size, hidden_size, kernel_size, bias = True,):
        ...
        self.conv2d = nn.Conv2d(in_channels = self.input_size + self.hidden_size, 
                                out_channels = 4 * self.hidden_size,
                                kernel_size = self.kernel_size,
                                padding = self.padding,
                                bias = self.bias)
        
    def forward(self, input_tensor, current_state, prints = True):
        
        h_state, c_state = current_state # hidden and cell states
        if prints: print ("input_shape into each neuron: ", input_tensor.shape,
                          "batch_size, channels, x-dim, y-dim")
        combined = torch.cat((input_tensor, h_state), dim=1) # concatenate xt and h(t-1)
        if prints: print ("combined input_shape: ", combined.shape)
        conv_comb = self.conv2d(combined)
        if prints: print ("conv_comb_shape: ", conv_comb.shape)
...
class ConvLSTM_layer(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, kernel_size, num_layers, 
        ...
        for l in range(0, self.num_layers):
            curr_input_size = self.input_size if l == 0 else self.hidden_size[l - 1]
            
            cell_list.append(ConvLSTMCell(input_size = curr_input_size,
        ...
        for layer_idx in range(self.num_layers):
            ...
            for t in range(seq_len):
                h, c = self.cell_list[layer_idx](input_tensor=cur_layer_input[:, t, :, :, :],
                                                 current_state = [h, c])
                output_inner.append(h) # list
            
              
            layer_output = torch.stack(output_inner, dim=1) # tensor
            #if prints: print("SHAPE of LAYER-output is: ", layer_output.shape)
            cur_layer_input = layer_output
            layer_output_list.append(layer_output)
            last_state_list.append([h, c])  
            #if prints: print("LAYER_output list len: ", len(layer_output_list))
            #
        layer_output_list = layer_output_list[-1:]
        last_state_list = last_state_list[-1:]
        
        if prints: print("LAYER_output list shape: ", layer_output_list[0].shape)


     
        # if prints: print("final ConvLSTM output shape:", last_state_tensor.shape, "[num_batches, seq_len, height, width]")
        # if prints: print ("type of the ConvLSTM output is: ", type(last_state_tensor))
        # if prints: print ("len of the ConvLSTM output: ", len(last_state_tensor))           
